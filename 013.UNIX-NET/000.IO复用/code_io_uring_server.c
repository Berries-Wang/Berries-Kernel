...
int main(int argc, char **argv) {
    ...
    int opt = 1;
    setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)); /* 设置地址重用，避免服务器重启时 "Address already in use" 错误 */
...
/*****************************************************************************
 * 附录：为什么每次处理完连接后要重新获取 SQE 并提交？
 *
 * 一、核心原因：io_uring 的 SQE 是"一次性消费"的
 *
 * 每个 SQE (Submission Queue Entry) 提交给内核后，内核处理完即将其从
 * 提交队列中移除，不会自动重新提交。如果想继续执行相同的操作（如继续
 * accept 新连接，或继续读某个客户端的数据），就必须：
 *
 *   获取新 SQE → 填充操作 → 设置上下文 → 提交
 *
 * 循环往复，形成"异步链"。
 *
 * 二、与 epoll 的对比
 ...
 * │ 模型     │ 行为                                                │
 * ├──────────┼──────────────────────────────────────────────────────┤
 * │ epoll    │ 只要 fd 上还有事件可处理，epoll_wait 会持续通知，   │
 * │ (LT模式) │ 不需要重新注册。                                    │
 * ├──────────┼──────────────────────────────────────────────────────┤
 * │ io_uring │ SQE 是一次性的。内核不会记住"下次还要继续 accept"， │
 * │          │ 每次请求完成 = 该生命周期结束，必须手动提交下一个。  │
 * └──────────┴──────────────────────────────────────────────────────┘
 *
 * 三、如果不重新提交会怎样？
 *
 *   - accept：服务器再也无法接受新连接，因为监听套接字上没有正在
 *     进行的 accept 请求。
 *   - read：该客户端发送的数据不会再被读取，因为 read 请求已完成，
 *     没有新的 read SQE 在队列中。
 *
 * 四、这会影响性能吗？—— 不会，反而更快
 *
 * 1. io_uring_get_sqe() 不是内存分配，只是将 SQ 的 tail 指针向前
 *    移动一位，从共享 ring buffer 中取出下一个槽位：
 *        sqe = &ring->sq.sqes[ring->sq.tail & ring->sq.ring_mask];
 *        ring->sq.tail++;
 *    纯内存操作，无锁，无系统调用，开销 ≈ 一个指针自增。
 *
 * 2. io_uring_submit() 支持批量提交，多个 SQE 可合并为一次系统调用。
 *    例如代码中第 302 行一次提交了 accept + read 两个 SQE。
 *
 * 3. 与 epoll 的真实开销对比：
 *
 *    ┌──────────┬──────────────────────┬──────────────────────────┐
 *    │ 操作     │ epoll                │ io_uring                 │
 *    ├──────────┼──────────────────────┼──────────────────────────┤
 *    │ 注册监听 │ epoll_ctl → 1 syscall │ 填 SQE → 合并提交分摊    │
 *    │ 等待事件 │ epoll_wait → 1 syscall │ io_uring_wait_cqe → 1   │
 *    │ 读数据   │ read() → 1 syscall    │ 已包含在 SQE 中，无需   │
 *    │ 写数据   │ write() → 1 syscall   │ 可提交 write SQE，无需  │
 *    └──────────┴──────────────────────┴──────────────────────────┘
 *
 *    epoll 的"注册一次"省了 epoll_ctl，但每次读写依然是独立 syscall；
 *    io_uring 的"每次提交"虽然需要重新填 SQE，但省掉了所有 read/write
 *    的系统调用——这才是真正的性能大头。
 *
 * 5. 总结
 *
 *    重新提交 SQE 不是性能开销，而是把"注册 + 读写"两步合并为"一步
 *    提交"的设计体现。io_uring 用"多一次 ring buffer 写入"换掉了
 *    "若干次 read/write 系统调用"，净赚。
 *
 *    实际基准测试中，io_uring 在大量 I/O 场景下比 epoll 有 30%-50%+
 *    的吞吐量提升，主要来源就是消除了每个 I/O 操作的 syscall 开销，
 *    以及批量提交减少了上下文切换。
 *****************************************************************************/