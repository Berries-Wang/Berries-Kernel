...
(SME).
(本文档简要概述了 Linux 为用户空间提供的接口，以支持 ARM 可伸缩向量扩展(SVE)的使用，包括与可伸缩矩阵扩展(SME)添加的流式 SVE 模式的交互。)
...
* In streaming mode FFR is not accessible unless HWCAP2_SME_FA64 is present
  ...
  (在流式模式下，FFR 不可访问，除非系统中存在 HWCAP2_SME_FA64；当不支持时，使用这些接口访问流式模式 FFR 时会将其读写为零。)
...
* On hardware that supports the SME extensions, HWCAP2_SME will also be
  ...
  for more details.
  (在支持 SME 扩展的硬件上，HWCAP2_SME 也将在 AT_HWCAP2 辅助向量条目中报告。除其他功能外，SME 添加了流式模式，该模式使用单独的 SME 向量长度和相同的 Z/V 寄存器提供 SVE 功能集的子集。更多详情请参见 sme.rst。)
...
* Debuggers should restrict themselves to interacting with the target via the
  ...
  in the target.
  (调试器应限制自己通过 NT_ARM_SVE 寄存器集与目标交互。检测此寄存器集支持的推荐方法是先连接到目标进程，然后尝试 ptrace(PTRACE_GETREGSET, pid, NT_ARM_SVE, &iov)。注意，当 SME 存在且流式 SVE 模式正在使用时，寄存器的 FPSIMD 子集将通过 NT_ARM_SVE 读取，并且 NT_ARM_SVE 写入将使目标退出流式模式。)
...
* On syscall, V0..V31 are preserved (as without SVE).  Thus, bits [127:0] of
  ...
  (在系统调用时，V0..V31 被保留(与没有 SVE 时一样)。因此，Z0..Z31 的位[127:0]被保留。Z0..Z31 的所有其他位，以及 P0..P15 和 FFR 的所有位在从系统调用返回时变为零。)
...
* The record contains a flag field which includes a flag SVE_SIG_FLAG_SM which
  ...
  length.
  (该记录包含一个标志字段，其中包括 SVE_SIG_FLAG_SM 标志，如果设置了该标志，则表示线程处于流式模式，且向量长度和寄存器数据(如果存在)描述的是流式 SVE 数据和向量长度。)
...
* It is permitted to enter or leave streaming mode by setting or clearing
  ...
  (允许通过设置或清除 SVE_SIG_FLAG_SM 标志来进入或离开流式模式，但应用程序应注意确保在此操作时 sve_context.vl 和任何寄存器数据适合新模式下的向量长度。)
...
* New regsets NT_ARM_SVE and NT_ARM_SSVE are defined for use with
  ...
  (定义了新的寄存器集 NT_ARM_SVE 和 NT_ARM_SSVE，用于 PTRACE_GETREGSET 和 PTRACE_SETREGSET。NT_ARM_SSVE 描述流式模式 SVE 寄存器，NT_ARM_SVE 描述非流式模式 SVE 寄存器。)
  ...
  (在本描述中，当目标处于适当的流式或非流式模式，并且正在使用超出与 FPSIMD Vn 寄存器共享的子集的数据时，寄存器集被称为"活跃"。)
...
(寄存器集数据以 struct user_sve_header 开始，包含：)
	...
	(如果既没有提供 FPSIMD 也没有提供 SVE 标志，则没有寄存器有效载荷可用，这仅在实现了 SME 时才可能。)
...
* In systems supporting SME when in streaming mode a GETREGSET for
  ...
  (在支持 SME 的系统中，当处于流式模式时，对 NT_REG_SVE 的 GETREGSET 将仅返回 user_sve_header，不包含寄存器数据；类似地，当不在流式模式时，对 NT_REG_SSVE 的 GETREGSET 也不会返回任何寄存器数据。)
...
* Where SME is implemented it is not possible to GETREGSET the register
  ...
  (在实现了 SME 的情况下，不可能在流式模式时 GETREGSET 普通 SVE 的寄存器状态，也不可能在普通模式时 GETREGSET 流式模式的寄存器状态，无论硬件在两种模式之间共享数据的实现定义行为如何。)

* Any SETREGSET of NT_ARM_SVE will exit streaming mode if the target was in
  streaming mode and any SETREGSET of NT_ARM_SSVE will enter streaming mode
  if the target was not in streaming mode.
  (如果目标处于流式模式，任何 NT_ARM_SVE 的 SETREGSET 将退出流式模式；如果目标不处于流式模式，任何 NT_ARM_SSVE 的 SETREGSET 将进入流式模式。)
...
* On systems that do not support SVE it is permitted to use SETREGSET to
  ...
  (在不支持 SVE 的系统上，允许使用 SETREGSET 通过 NT_ARM_SVE 写入 SVE_PT_REGS_FPSIMD 格式的数据，在这种情况下，向量长度应指定为 0。这允许在具有 SME 但没有 SVE 的系统上禁用流式模式。)
...
* If any register data is provided along with SVE_PT_VL_ONEXEC then the
  ...
  (如果随 SVE_PT_VL_ONEXEC 一起提供了任何寄存器数据，则寄存器数据将使用当前向量长度进行解释，而不是使用配置为在 exec 时使用的向量长度。)
...
* NT_ARM_SVE and NT_ARM_SSVE notes will be added to each coredump for
  ...
  (将为转储进程的每个线程向每个核心转储添加 NT_ARM_SVE 和 NT_ARM_SSVE 注释。其内容等效于在生成核心转储时对每个线程执行相应类型的 PTRACE_GETREGSET 所读取的数据。)
...
/proc/sys/abi/sve_default_vector_length
    ...
    (将整数的文本表示写入此文件会将系统默认向量长度设置为指定值，并使用与通过 PR_SVE_SET_VL 设置向量长度相同的规则四舍五入到支持的值。)
...
* The arm64 specific DWARF standard [5] added the VG (Vector Granule) register
  ...
  (arm64 特定的 DWARF 标准[5]在索引 46 处添加了 VG(向量粒度)寄存器。当可变长度的 SVE 寄存器被压入栈时，此寄存器用于 DWARF 展开。)

* Its value is equivalent to the current SVE vector length (VL) in bits divided
  by 64.
  (其值等于当前 SVE 向量长度(VL)(以位为单位)除以 64。)
...
* The value is included in Perf samples in the regs[46] field if
  ...
  (如果设置了 PERF_SAMPLE_REGS_USER 且 sample_regs_user 掩码设置了位 46，则该值将包含在 Perf 采样的 regs[46] 字段中。)
...
* If the system doesn't support SVE when perf_event_open is called with these
  ...
  (如果系统不支持 SVE，在使用这些设置调用 perf_event_open 时，事件将无法打开。)
