...
struct file_operations {
	...
	/* 轮询一个已提交的异步 IO 请求(kiocb)是否完成,spin 为真时自旋等待;
	 * 用于 io_uring 的 polled IO(不依赖完成中断) */
	int (*iopoll)(struct kiocb *kiocb, bool spin);
	/* 遍历目录,实现 getdents(2):对每个目录项调用 ctx 中的回调填充给用户态;
	 * 持有目录 inode 的独占锁,仅目录文件实现 */
	int (*iterate) (struct file *, struct dir_context *);
	/* 同 iterate,但只持共享锁,允许并发遍历同一目录;
	 * 现代文件系统应优先实现它 */
	...
	/* 声明本文件 mmap 支持的 flags(MAP_POPULATE/MAP_SYNC 等),
	 * VFS 用它校验 mmap 参数 */
	...
	/* sendfile 零拷贝路径:把内核 page 直接写入目标文件/socket,
	 * 避免数据在内核与用户态之间复制 */
