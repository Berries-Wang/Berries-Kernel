 ...
 */
static inline int copy_present_pte(struct mm_struct *dst_mm,
				   struct mm_struct *src_mm, pte_t *dst_pte,
				   pte_t *src_pte, struct vm_area_struct *vma,
				   ...
				   struct page **prealloc)
{
	...
	if (page) {
		...
		retval =
			copy_present_page(dst_mm, src_mm, dst_pte, src_pte, vma,
					  new, addr, rss, prealloc, pte, page);
		if (retval <= 0)
			return retval;
...
static int copy_pte_range(struct mm_struct *dst_mm, struct mm_struct *src_mm,
			  ...
			  struct vm_area_struct *vma,
			  struct vm_area_struct *new, unsigned long addr,
			  unsigned long end)
{
...
again:
	...
	do {
		...
		if (unlikely(!pte_present(*src_pte))) {
			entry.val =
				copy_nonpresent_pte(dst_mm, src_mm, dst_pte,
		...
		ret = copy_present_pte(dst_mm, src_mm, dst_pte, src_pte, vma,
				       new, addr, rss, &prealloc);
		/*
...
static inline int copy_pmd_range(struct mm_struct *dst_mm,
				 struct mm_struct *src_mm, pud_t *dst_pud,
				 pud_t *src_pud, struct vm_area_struct *vma,
				 struct vm_area_struct *new, unsigned long addr,
				 unsigned long end)
{
	...
	do {
		...
		if (pmd_none_or_clear_bad(src_pmd))
			continue;
		if (copy_pte_range(dst_mm, src_mm, dst_pmd, src_pmd, vma, new,
				   addr, next))
			return -ENOMEM;
...
static inline int copy_pud_range(struct mm_struct *dst_mm,
				 struct mm_struct *src_mm, p4d_t *dst_p4d,
				 p4d_t *src_p4d, struct vm_area_struct *vma,
				 struct vm_area_struct *new, unsigned long addr,
				 unsigned long end)
{
	...
	do {
		...
		if (pud_none_or_clear_bad(src_pud))
			continue;
		if (copy_pmd_range(dst_mm, src_mm, dst_pud, src_pud, vma, new,
				   addr, next))
			return -ENOMEM;
...
static inline int copy_p4d_range(struct mm_struct *dst_mm,
				 struct mm_struct *src_mm, pgd_t *dst_pgd,
				 pgd_t *src_pgd, struct vm_area_struct *vma,
				 struct vm_area_struct *new, unsigned long addr,
				 unsigned long end)
{
	...
	do {
		...
		if (p4d_none_or_clear_bad(src_p4d))
			continue;
		if (copy_pud_range(dst_mm, src_mm, dst_p4d, src_p4d, vma, new,
				   addr, next))
			return -ENOMEM;
...
/**
 * 参考资料: [Run Linux Kernel (2nd Edition) Volume 1: Infrastructure.epub]#4.7.7　写时复制
 * 
 * 当用户试图修改(就是写嘛)只有只读属性的页面时，CPU触发异常，在do_wp_page()函数里尝试修复这些页面，
 * 通常做法是新分配一个页面并且复制旧页面内容到新的页面中，这个新分配的页面具有可写的属性。
 * 
...
static vm_fault_t do_wp_page(struct vm_fault *vmf) __releases(vmf->ptl)
{
	...
	/**
	 * vm_normal_page 查找地址vmf->address对应页面的page数据结构，
	...
	// 执行到此处，说明page就是一个普通映射页面

	/*
	 * Take out anonymous pages first, anonymous shared vmas are
	 * not dirty accountable.
	 * 
	 * PageAnon()宏用于判断是否匿名页面
	 */
					    ...
					    struct zap_details *details)
{
	...
	vma_interval_tree_foreach(vma, root, details->first_index,
				  details->last_index)
	{
		vba = vma->vm_pgoff;
...
/**
 * 见: [Run Linux Kernel (2nd Edition) Volume 1: Infrastructure.epub]#图4.35　do_anonymous_page()函数的流程
 * 
 * We enter with non-exclusive mmap_lock (to exclude vma changes,
 * but allow concurrent faults), and pte mapped but not yet locked.
 * We return with mmap_lock still held, but pte unmapped and unlocked.
...
static vm_fault_t do_anonymous_page(struct vm_fault *vmf)
{
	 ...
	 * 
	 * 这里使用pte_offset_map()可能不安全，可能其他的内核线程（如khugepaged内核线程）会创建一个巨大的pmd页表项，
	 * 因为我们当前只持有读者类型的mmap_sem信号量
	 *
	 * Here we only have mmap_read_lock(mm).
	 * 
	 * pte_alloc : 创建pte，并复制到pmd页表项中
	 * 
	 * 像if(xx) (xx为int类型时), 只有xx==0时，才是false，否则都是true(不论正负数)
	 */
	...
	/**
	 *  Use the zero-page for reads   处理可读的情况
	 * !(vmf->flags & FAULT_FLAG_WRITE) 表示没有写权限
	 * 
	 * my_zero_pfn()获取系统零页的帧号
	 * pte_mkspecial()宏用来设置PTE中的PTE_SPECIAL位，表示这是特殊映射页面
	 * 
	 * 
	 * 这个系统零页面，比较特别? you see ,在只读的时候，并没有实际分配物理页面,
	...
	/**
	 * 分配一个可移动的匿名页: 通过伙伴系统
	 * 000.LINUX-5.9/include/linux/highmem.h
	 */
...
static vm_fault_t handle_pte_fault(struct vm_fault *vmf)
{
	...
	// PTE 已经存在，但是页面不在内存中,PTE还没有映射物理页面，是真正的缺页
	...
	/**
	 * 调用pte_protnone()说明页面被设置为NUMA调度的页面
	 */
	...
	if (vmf->flags & FAULT_FLAG_WRITE) {
		/**
		 * 如果处理器因为写内存而触发缺页异常，并且PTE是只读属性，就会触发一个写时复制的缺页中断，
		 * 调用do_wp_page()函数。如父、子进程之间共享的内存，当其中一方需要写入新内容时，
		 * 就会触发写时复制。
		 * 
		 * > 父子进程之间，不是线程之间!
		 */
		...
		// 如果当前PTE的属性是可写的，那么通过pte_mkdirty()函数来设置PTE_DIRTY位
		entry = pte_mkdirty(entry);
	}
	// pte_mkyoung()对于ARM64架构会设置PTE_AF位。
	entry = pte_mkyoung(entry);
				    ...
				    unsigned long address, unsigned int flags)
{
	...
	// 本次触发缺页的操作是否是一次写操作 (如果是写，那么写了之后就变dirty了)
			   ...
			   unsigned int flags, struct pt_regs *regs)
{
	 ...
	 * (在进入真正的临界区（critical section）之前，先完成计数器的更新)
	 *  */
	 ...
	 * (对于在用户空间触发的错误（faults），启用 memcg OOM 处理机制。
	 * 而内核空间的错误则会得到更优雅的处理)
	 */
			     ...
			     pte_t **ptepp, spinlock_t **ptlp)
{
	...
	(void)__cond_lock(*ptlp, !(res = __follow_pte_pmd(mm, address, NULL,
							  ptepp, NULL, ptlp)));
	return res;
}
		   ...
		   pmd_t **pmdpp, spinlock_t **ptlp)
{
	...
	/* (void) is needed to make gcc happy */
	(void)__cond_lock(*ptlp, !(res = __follow_pte_pmd(mm, address, range,
							  ptepp, pmdpp, ptlp)));
	return res;
}
